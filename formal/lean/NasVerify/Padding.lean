...
`formal/check.sh` appends a `#print axioms` command for **every** theorem
declared above and fails on anything outside `propext`, `Classical.choice` and
`Quot.sound`.

The commands are generated by the gate rather than written here on purpose. An
earlier version read whatever `#print axioms` lines this file happened to
contain, which meant it only checked theorems that volunteered their axioms — a
review planted an `axiom`-backed theorem with no print line and the gate
reported "axioms clean". A gate you can opt out of is not a gate. -/
