...
check         "peer is reachable over a Tor onion carrier"       $NAS peer status home-nas
check         "writes are accepted with no peer reachable"       $NAS test offline-write work
check         "the outbox replays and re-merges on reconnect"    $NAS test outbox-replay work
check         "a CAS conflict on replay merges, not errors"      $NAS test outbox-conflict-merges work
